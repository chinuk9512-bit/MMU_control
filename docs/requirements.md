...
The application stores settings, command sets, and automation scenarios under `%APPDATA%\MMUControl` on Windows. If `APPDATA` is not defined, it falls back to `~/AppData/Roaming/MMUControl`.

Expected files include:

- `settings.json` - SSH, Board/MMU, Power Supply, active profile name, and window state
- `%APPDATA%\MMUControl\command_sets.json` - saved command sets from the Commands tab
- `%APPDATA%\MMUControl\automation_scenarios.json` - saved automation scenarios
