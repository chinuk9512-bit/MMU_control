...
기본 사용자 데이터 위치는 `%APPDATA%/MMUControl`입니다. `APPDATA`가 없으면 홈 디렉터리 아래 `AppData/Roaming/MMUControl`을 사용합니다.
...
- `MMUControl.spec`은 package resource인 `power_supply_commands.json`을 포함하도록 구성됩니다.
